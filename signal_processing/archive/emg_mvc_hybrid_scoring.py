"""MVC Hybrid Segmentation with Evidence-Driven Scoring System.

ARCHIVED: This module contains a sophisticated MVC segmentation pipeline that was
developed for the PrevOccupAI+ project but deemed more complex than necessary for
that project's requirements. It is preserved here for use in future projects that
require optimal MVC segment detection.

Key Features:
- Evidence-driven threshold selection (compares multiple candidate thresholds)
- Scoring system that evaluates segmentations based on MVC protocol expectations
- TKEO-based energy extraction with log transform
- Robust baseline estimation with dropout detection
- Adaptive sigma calculation to handle zero-inflated baselines

The main entry point is `detect_mvc_segments_hybrid()` which returns both the
detected segments and detailed debug info for visualization.

For simpler use cases where you just need >=2 segments, use the standard
`detect_mvc_segments_tkeo()` from `signal_processing.emg_mvc` instead.

Reference: 
- Solnik et al. (2008) "Teager-Kaiser Operator improves the accuracy of EMG onset detection"
- See docs/mvc_segmentation_pipeline_guide.md for detailed algorithm explanation
"""
...
from sensors.process.emg_preprocessing import bandpass_filter, tkeo
...
def _score_segmentation(
    ...
    Scoring breakdown:
    - Base score: 2 segments = +25, 3 = +15, 4+ = penalized, <2 = negative
    - Duration: 1-3s = +5 (ideal), 0.5-5s = +2 (acceptable), >10s = heavy penalty
    - Contrast: High contrast between top-2 and baseline = bonus
    - Penalties: Overlap with baseline, edge effects, clustering
    ...
    for start, end in top_2:
        ...
        if 1.0 <= duration_s <= 3.0:
            ...
            score -= 20.0 - 2.0 * (duration_s - 10.0)
    ...
    # Penalty for baseline overlap
    for start, end in top_2:
        if start < baseline_end and end > baseline_start:
            score -= 15.0
    
    # Penalty for edge effects
    for start, end in top_2:
    ...
    if len(top_2) >= 2:
        ...
        if separation > signal_length * 0.3:
            score += 5.0
...
def detect_mvc_segments_hybrid(
    ...
    5. Score each result based on MVC expectations (2 segments, ~1-3s each)
    ...
    - Guards against zero-inflated baselines (adaptive min sigma)
    - Excludes padded edges from baseline search
    - Dropout detection in linear energy space
